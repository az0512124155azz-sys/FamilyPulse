...
export default function App(){
  ...
  const children=useMemo(
    ()=>members.filter(m=>m.role==='child' && m.active!==false),
    [members]
  );
  ...
  useEffect(()=>{
    ...
    return onSnapshot(collection(db,'families',profile.familyId,'logoutEvents'),snap=>{
      ...
      if(newest?.loggedOutAt?.seconds){
        const age=Date.now()-newest.loggedOutAt.seconds*1000;
        if(age<15000 && 'Notification' in window && Notification.permission==='granted'){
          navigator.serviceWorker?.ready
            .then(reg=>reg.showNotification(`${newest.name} התנתק מ-FamilyPulse`,{
              body:newest.hasLocation?'המיקום האחרון נשמר.':'לא הצלחנו לקבל מיקום אחרון.',
              icon:newest.photoURL||'/icon.svg'
            }))
            .catch(()=>{});
        }
      }
  ...
  useEffect(()=>{
    ...
    return onSnapshot(reqRef,async snap=>{
      ...
      try{
        ...
        try{
          const fast=await getPosition({enableHighAccuracy:false,timeout:1200,maximumAge:300000});
        ...
        try{
          const precise=await getPosition({enableHighAccuracy:true,timeout:7000,maximumAge:15000});
          await setDoc(doc(db,'locations',profile.uid),{
  ...
  async function connectCode(){
    ...
    if(data.type==='child'){
      if(profile.role!=='parent' || !profile.familyId){
        setMessage('רק הורה יכול לצרף ילד.');
        return;
      }
      ...
      if(profile.role!=='parent' || !data.familyId){
        setMessage('הקוד הזה אינו קוד שיתוף פעיל.');
        return;
      }
    ...
    if(profile?.role==='child'){
      ...
      try{
        ...
        if(familyId){
          ...
          await updateDoc(
            doc(db,'families',familyId,'members',profile.uid),
            {
              active:false,
  ...
  async function enableNotifications(){
    if(!('Notification' in window)){
      setMessage('הדפדפן הזה לא תומך בהתראות Web.');
      return;
    }
    const result=await Notification.requestPermission();
    setMessage(result==='granted'?'ההתראות הופעלו.':'לא ניתנה הרשאה להתראות.');
  }

  if(loading) return <Center><div className="loader"/><p>טוען את FamilyPulse…</p></Center>;
  if(!firebaseReady) return <Center><Logo/><h1>FamilyPulse</h1><p>יש להגדיר את Firebase.</p></Center>;
  ...
  if(profile.role==='child') return <div className="childPage">
    ...
    <main className="childMain">
      ...
      <h1>הכול מחובר</h1>
      <p>כשהורה מבקש מיקום בזמן שהאפליקציה פתוחה, FamilyPulse שולח קודם מיקום מהיר ואז משפר אותו.</p>
      <CodeCard code={profile.code} title="קוד החיבור שלך"/>
      <div className="infoBox"><Smartphone/><span>בעת התנתקות ננסה לשמור להורה מיקום אחרון. אם הרשאת המיקום חסומה, ההתנתקות עדיין תתבצע ללא מיקום.</span></div>
      {message&&<div className="toast">{message}</div>}
    </main>
  </div>;
  ...
  return <div className="appShell">
    ...
    <main className="dashboard">
      <section className="hero">
        <div>
          <span className="eyebrow">המשפחה שלי</span>
          <h1>שלום, {profile.name}</h1>
          <p>{children.length} ילדים · {parents.length} הורים מחוברים</p>
        </div>
        <div className="heroActions">
          {'Notification' in window && Notification.permission!=='granted' &&
            <button className="secondary compact" onClick={enableNotifications}>אפשר התראות</button>}
          <div className="avatar big">{profile.photoURL?<img src={profile.photoURL}/>:profile.name[0]}</div>
        </div>
      </section>
        ...
        <div className="logoutNoticeList">
          {logoutEvents.map(event=><article className="logoutNotice" key={`${event.childUid}-${event.loggedOutAt?.seconds||0}`}>
            <div className="avatar">
              {event.photoURL?<img src={event.photoURL}/>:event.name[0]}
            </div>
            <div className="grow">
              ...
              {event.hasLocation&&typeof event.lat==='number'&&typeof event.lng==='number'?
                <small>
                  מיקום אחרון: {event.lat.toFixed(5)}, {event.lng.toFixed(5)}
                  {typeof event.accuracy==='number'?` · דיוק כ־${Math.round(event.accuracy)} מ׳`:''}
                </small>:
            ...
            {event.hasLocation&&typeof event.lat==='number'&&typeof event.lng==='number'&&
              <button
                className="locate"
                onClick={()=>window.open(`https://www.google.com/maps?q=${event.lat},${event.lng}`,'_blank','noopener,noreferrer')}
              >
                <MapPin/> פתח מיקום
              </button>}
          </article>)}
      ...
      <section>
        <div className="sectionTitle"><h2>הילדים</h2><span>{children.length}</span></div>
        {children.length===0?<div className="empty"><Baby/><h3>עוד אין ילדים פעילים</h3><p>חבר ילד באמצעות הקוד שלו.</p></div>:
                ...
                <Popup>
                  <div className="mapPopup" dir="rtl">
                    <div className="mapPopupHeader">
                      <div className="avatar">
                        {child.photoURL?<img src={child.photoURL}/>:child.name[0]}
                      </div>
                      <div>
                        <b>{child.name}</b>
      ...
      <section className="share"><Users/><div className="grow"><h2>הורה שותף</h2><p>הורה נוסף בוחר “אני הורה” ומקליד את הקוד שלך.</p></div><CodeCard code={profile.code} compact/></section>
    </main>
  </div>;
}
...
function CodeCard({code,title,compact=false}:{code:string;title?:string;compact?:boolean}){
  ...
  return <div className={compact?'codeCard compactCode':'codeCard'}>
    {title&&<span>{title}</span>}
    <strong>{code}</strong>
    <button onClick={copy} aria-label="העתקת קוד"><Copy/></button>
  </div>
}

function TopBar({profile,onLogout}:{profile:Profile;onLogout:()=>void}){
  return <header className="topbar">
    <div className="brand"><Logo/><b>FamilyPulse</b></div>
    <div className="miniProfile">
      <span>{profile.role==='parent'?'הורה':'ילד/ה'}</span>
      <div className="avatar tiny">{profile.photoURL?<img src={profile.photoURL}/>:profile.name[0]}</div>
      <button className="logoutButton" onClick={onLogout}>התנתק</button>
    </div>
  </header>
}

function locationAge(updatedAt?:{seconds:number}){
  if(!updatedAt?.seconds) return 'עודכן עכשיו';
...
function createChildMarkerIcon(child:Member,selected:boolean){
  ...
  const image=child.photoURL?.startsWith('data:image/')
...
function MapViewport({
  ...
  useEffect(()=>{
    ...
    if(points.length===1){
      ...
      map.fitBounds(latLngBounds(points),{padding:[56,56],maxZoom:15,animate:true});
    }
  },[
    map,
    selected?.uid,
    fitSignal,
    childrenList.map(c=>c.uid).join('|'),
    Object.values(locations).map(l=>`${l.lat},${l.lng},${l.updatedAt?.seconds||0}`).join('|')
  ]);
